<template>
    <ap-modal
        :isOpen.sync="$parent.$data.withdrawModal"
        :isCenter="true">

        <ap-modal-body>
            <ap-modal-title>
                Withdraw Call Credit Balance
            </ap-modal-title>
    ...
    methods:
    {
        async submit()
        {
            ...
            const response = await Http.post(
                `/administration/company/${this.$route.params.code}/call/credit`,
                this.form,
            );
            ...
            {
                this.$parent.$data.summary.call_credit = parseFloat(this.$parent.$data.summary.call_credit) - parseFloat(response.data.amount);
        ...
        closeModal()
        {
            this.$parent.toggleModal('withdraw', false);
        },
    },
};
</script>
