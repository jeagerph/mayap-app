    ...
    <div class="ap-animate ap-animate-fade-in">
        <div class="card radius-10">
            <div class="card-body">
                ...
                <router-link
                    class="ap-link-reset"
                    :to="`/administration/invoices?filter[brgyCode]=${slugCode}&filter[status]=1`">
                    <ap-grid>
                        <div class="ap-width-expand">
                            <small>
                                NOT YET DELIVERED
                ...
                <router-link
                    class="ap-link-reset"
                    :to="`/administration/invoices?filter[brgyCode]=${slugCode}&filter[status]=1&dueDate=1`">
                    <ap-grid>
                        <div class="ap-width-expand">
                            <small>
                                DUE DATE
...
export default
{
    ...
    computed:
    {
        ...
        slugCode()
        {
            return this.barangay.slug
                ? this.barangay.slug.code
                : 'DELETED'
        },
    },
    ...
    methods:
    {
        async loadSummary()
        {
            ...
            const response = await Http.get(`/administration/barangay/${this.$route.params.code}/summary/billings?from=${date.firstDate}&to=${date.lastDate}`);
