<template>
    <ap-modal
        :isOpen.sync="$parent.$data.updateCityLogoModal"
        :isCenter="true">
        <ap-modal-body>
            <ap-modal-title>
                Update City Logo
            ...
            <div class="ap-margin-top ap-margin-bottom ap-text-center">
                <label class="ap-upload-label-button" for="upload-city-image">Select Photo</label>
                <input class="ap-display-none" id="upload-city-image" type="file" accept="image/**" @change="onFileChange">
                <ap-input
    ...
    methods:
    {
        ...
        async upload()
        {
            ...
            const response = await this.update({
                url: `/administration/company/${this.$route.params.code}/barangay/${this.companyBarangay.id}/city/logo`,
                form: formPhoto,
            });
            ...
            if (response.status === 200)
            {
                ...
                Toast.success({
                    message: 'City logo has been updated.'
                });
            }
        ...
        closeModal()
        {
            this.$parent.toggleModal('updateCityLogo', false);
        },
