<template>
    <ap-modal
        :isOpen.sync="$parent.$data.updateBarangayLogoModal"
        :isCenter="true">
        <ap-modal-body>
            <ap-modal-title>
            ...
            <div class="ap-margin-top ap-margin-bottom ap-text-center">
                <label class="ap-upload-label-button" for="upload-barangay-image">Select Photo</label>
                <input class="ap-display-none" id="upload-barangay-image" type="file" accept="image/**" @change="onFileChange">
                <ap-input
...
export default
{
    ...
    {
        ...mapGetters({
            companyBarangay: 'adminCompanyBarangay/selected',
    ...
    {
        ...mapActions({
            update: 'adminCompanyBarangay/update',
        }),
        ...
        async upload()
        {
            ...
            {
                formPhoto.logo = output;
            });

            const response = await this.update({
                url: `/administration/company/${this.$route.params.code}/barangay/${this.companyBarangay.id}/barangay/logo`,
                form: formPhoto,
            });
            ...
            if (response.status === 200)
            {
                ...
                Toast.success({
                    message: 'Barangay logo has been updated.'
                });
            }
        ...
        closeModal()
        {
            this.$parent.toggleModal('updateBarangayLogo', false);
        },
