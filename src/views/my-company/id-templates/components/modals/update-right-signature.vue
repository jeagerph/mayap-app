<template>
    <ap-modal
        :isOpen.sync="$parent.$data.updateRightSignatureModal"
        :isCenter="true">
        <ap-modal-body>
            <ap-modal-title>
                Update Right Signature
            </ap-modal-title>
            ...
            <div class="ap-margin-top ap-margin-bottom ap-text-center">
                ...
                <label class="ap-upload-label-button" for="upload-right-signature-image">Select Signature</label>
                <input class="ap-display-none" id="upload-right-signature-image" type="file" accept="image/**" @change="onFileChange">
                <ap-input
...
export default
{
    // mounted()
    // {
    //     this.$refs.rightSignatureRef.bind({
    ...
    {
        ...mapGetters({
            template: 'myBarangayTemplate/selected',
            isLoading: 'myBarangayTemplate/isLoading',
        }),
    },
    ...
    {
        ...mapActions({
            update: 'myBarangayTemplate/update',
        }),
        ...
        async upload()
        {
            ...
            await this.$refs.rightSignatureRef.result(options, (output) =>
            {
                formPhoto.photo = output;
            });

            const response = await this.update({
                url: `/my-barangay/template/${this.$route.params.code}/right-signature`,
                form: formPhoto,
            });
        ...
        onFileChange(e)
        {
            ...
            {
                vm.$refs.rightSignatureRef.bind({
                    url: response,
                });
            });
        },
        ...
        {
            this.$refs.rightSignatureRef.rotate(rotationAngle);
        },
        ...
        closeModal()
        {
            this.$parent.toggleModal('updateRightSignature', false);
        },
